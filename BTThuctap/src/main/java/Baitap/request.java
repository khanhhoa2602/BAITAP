	...
	public void run() {			
		...
		try {
				...
				if(200<=statuscode&&statuscode<=399) {
					pingok=pingok+1;
					error=error+0;
					System.out.println("Ping to url: "+url+"--statuscode: "+statuscode+"--truy cap binh thuong");
				...
				if(error==3) {
					log.error("url "+url+" -loi");//nếu 3 lần error thì hiển thị ra màn hình và lưu vào file cấu hình
					break;
				}
				if(pingok==3) {
